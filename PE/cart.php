...
<?php
    if(isset($_SESSION["idUtente"])){
      $idUtente=$_SESSION["idUtente"];
        ?>
<div class="container" style="border: 3px solid #4CAF50; border-radius: 20px;">
  <table>
    <thead>
      <tr>
        <th scope="col" style="color:red">Elimina</th>
        <th scope="col" style="color:red">Immagine</th>
        <th scope="col" style="color:red">Nome</th>
        <th scope="col" style="color:red">Colore</th>
        <th scope="col" style="color:red">Taglia</th>
        <th scope="col" style="color:red">Quantità</th>
        <th scope="col" style="color:red">Prezzo</th>
        <th scope="col" style="color:red">Prezzo*Qta</th>
      </tr>
    </thead>
    <tbody>
      <?php
      ...
      $result = $mysql->query("SELECT idCart, qta, taglia, prodotto.colore, prodotto.nome, prodotto.prezzo, foto.foto1
      FROM cart
      INNER JOIN prodotto ON prodotto.idProdotto=cart.idProdotto
      INNER JOIN foto ON foto.idFoto=prodotto.idFoto
      where cart.idUtente= $idUtente");
      ...
      if ($result->num_rows > 0) {
        while ($row = $result->fetch_assoc()) {
          ...
          $foto1 = $row['foto1'];
          $idCart= $row['idCart'];
          $prezzoPro=$prezzo*$qta;
          $tot=$tot+($prezzo*$qta);
          ?>
          <tr>
            <td scope="row"><?php echo"<a href='cart_del.php?idCart=$idCart' class='btn btn-danger'>X</a>";?></td>
            <td><img style=width:80%; src="./images/<?php echo $foto1; ?>"></td>
            <td><p style="color: #4CAF50;"><?php echo $nome; ?></p></td>
            <td><p style="color: #4CAF50;"><?php echo $colore; ?></p></td>
            <td><p style="color: #4CAF50;"><?php echo $taglia; ?></p></td>
            <td><p style="color: #4CAF50;"><?php echo $qta; ?></p></td>
            <td><p style="color: #4CAF50;"><?php echo $prezzo; ?></p></td>
            <td><p style="color: #4CAF50;"><?php echo $prezzoPro; ?></p></td>
          </tr>
        <?php
        } 
      } else {
...
<div class="container" style="border: 3px solid #4CAF50; border-radius: 20px;">
<?php echo "<p style='color: #4CAF50;'>Costo totale: ". $tot."€</p>"; ?>
</div>
    <?php }else{
    $_SESSION["err"] = 5;
